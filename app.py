...
class TumaMpesaService:
    ...
    async def initiate_payment(self, amount: float, phone: str, reference: str = None) -> dict:
        """Initiate MPesa payment via Tuma"""
        if not reference:
            ...
            try:
                response = await client.post(
                    ...
                    json={
                        ...
                        "phone": phone,
                        "reference": reference,
                        "callback_url": f"https://pharmacyos-1.onrender.com/api/payment/callback"
                    },
                    timeout=30.0
                )
...
async def payment_callback(request: Request):
    """Webhook callback for Tuma payment status"""
    try:
        data = await request.json()
        
        # Verify webhook signature (implement based on Tuma's documentation)
        payment_id = data.get("payment_id")
        status = data.get("status")
        reference = data.get("reference")
